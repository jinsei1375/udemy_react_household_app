...
export default function TransactionTable() {
  ...
  const visibleRows = React.useMemo(() => {
    const sortedMonthlyTrsanctinos = [...monthlyTransactions].sort((a, b) =>
      compareAsc(parseISO(a.date), parseISO(b.date))
    );
    return sortedMonthlyTrsanctinos.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <Grid container sx={{ borderBottom: '2px solid #eee' }}>
          ...
          <FinancialItem title={'収入'} value={balance} color={theme.palette.balanceColor.main} />
        </Grid>
